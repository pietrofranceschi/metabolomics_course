...
As we discussed in the lecture, metabolites produce peaks in the LC-MS 2D data, but a single metabolite is likely to produce a large number of peaks due to the nature of the ionization process occuring inside the MS interface. 
...
* each algorith has a specific set of parameters which should be optimized to get out the best
* each algorithm will make errors
* a suboptimal choice at this stage cannot be "compensated" further on

... so be careful, and take your time to strive for "the best"



First of all let's read in ine of our apple injections
...
After this step, the peak detection is strightforward
...
* What happens if you tweek the parameters (Warning: do not reduce too much the bin size ;-))?
* Many of the peaks are arranged in vertical lines ... why?
...
* prefilter c(k, I): this seemingly obscure couple of numbers is used to select the singnals which will be subjected to ROI detection. Mass traces will be teken into consideration only if they contain k peaks with intensity higher than I.
...
* What happens if you tweek the parameters?
